...
interface UseDataTableProps<TData, TValue> {
  ...
  /**
   * Disable pagination.
   * To be used when the table is not paginated.
   * @default false
   * @type boolean
   */
...
export function useDataTable<TData, TValue>({
  ...
  const pagination = React.useMemo(
    () => ({
      ...
      pageSize: disablePagination ? data.length : pageSize,
    }),
    [disablePagination, pageIndex, pageSize, data.length]
  )
  ...
  const table = useReactTable({
    ...
    pageCount: disablePagination ? 1 : pageCount,
    state: {
      pagination,
      sorting,
      columnVisibility,
